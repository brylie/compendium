...
- A **Collection** is a set of records that share a property schema (e.g., Status, Due Date, Owner) — this is the structured-data surface. That schema is entirely user-defined: there is no mandatory field, including a title or a rich-text body — a property exists only if a person explicitly adds it to that Collection. This is a deliberate departure from CMS platforms (early Joomla, pre-8 Drupal) that force every content type onto a fixed title+body node shape whether or not it fits.
- A **View** is a renderer over a Collection's records: Table, Kanban (grouped by a select property), Calendar (positioned by a date property), Gallery, etc. Views do not own data — they query it. Adding a new view type later is a rendering change, not a data migration. ("View" here always means this — a database/Collection view, Notion's sense of the word — never an MVC-style page or route. There is deliberately no standalone "view route": a View exists only as configuration on a `collection_view` block embedded in a Document, per `docs/specifications/collection-views.md`.)
...
- A slash-command menu (typing `/`) inserts or transforms blocks — heading, list, to-do checklist, toggle/collapsible, quote, callout, divider, table, table of contents, synced block, page link, code, embed — following the convention established by Notion, Coda, and WordPress's Gutenberg block inserter. As the catalog grows past a flat list's usability (0.4.0 alone adds columns, math, diagram, and bookmark), the menu groups commands by category (e.g., Writing, Structure, Media, Data, Reuse), supports searchable aliases per command, and shows a short description — matching Notion's and Gutenberg's own categorized inserters rather than one long undifferentiated list.
...
- _Acceptance:_ Given a source block and one or more synced instances of it, when content changes at the source or at any synced instance, then every location converges to identical content with no location left showing stale content.
...
- **Collection record detail pane** — opening a record from Table, Board, or Calendar reveals it as a rich knowledge object (whichever properties that Collection's schema actually defines, plus backlinks/relations and activity/attribution) rather than only an inline cell/card edit. The pane renders the record's existing schema properties and introduces no field of its own: a Collection with no rich-text property simply has no body section in its pane, consistent with Collections having no enforced schema (see Core Architectural Principle). If richer per-record content is wanted, that's an explicit property a person adds to that Collection's schema, not a universal field forced onto every record. This keeps the pane a rendering surface, not a second record or a separate data path, per the Unified record data model's own acceptance criterion that a record is addressable through the same record API everywhere.
...
- **Columns/layout block** — a multi-column container for arranging block content side-by-side on a page, validated as a near-universal editor feature: Notion's column blocks, Confluence's page-layout/panel system, and WordPress Gutenberg's Columns block all converge on it independently. Required block-model support for nested child-block containers — the first block type to need it _(corrected 2026-09-06: this item previously and incorrectly claimed columns would reuse an existing toggle nesting pattern; corrected 2026-09-14: toggle/collapsible has since gained the same real child-block containment as its own follow-up, issue #227 — the two block types now share one generic container mechanism rather than columns being the only one)_. Must degrade gracefully on narrow viewports (columns stack) per the accessibility/responsive acceptance bar (see Requirements intro).
- **Math/equation block** (LaTeX, e.g. via KaTeX) — inline and block-level equation rendering; valuable for the technical/engineering-heavy target persona and validated in Notion's block set.
- **Diagram block (Mermaid)** — renders Mermaid diagram syntax (flowcharts, sequence diagrams, and similar) from a stored text block, the same convention GitHub and GitLab both render natively in Markdown (GitLab has supported it since version 10.3). The practical, low-architectural-cost stand-in for the diagramming use case an edgeless canvas would otherwise serve — see Future Considerations.
- **Bookmark / Smart Link block** — pasting a URL offers a rich preview card (title, description, favicon/thumbnail where available) distinct from the plain `embed` block, the way Confluence's Smart Links work; always retains an accessible plain link as a fallback.
- **Block action menu and document outline (List View)** — every block gets a hover/focus handle exposing duplicate, delete, convert, copy-link, and move-up/down actions, with multi-select and group-as-a-unit support; a document-level List View shows heading hierarchy and every block in order, and selecting an entry focuses and scrolls to it. Complements the drag-and-drop reordering and slash-command/toolbar conversion already specified in Block editor interaction above.
- **Synced-block provenance and detach** — a synced block visibly shows it's shared (source badge), lists every location currently using it, and offers a safe "detach to independent copy" action with a warning before an edit that will propagate everywhere else. Particularly important when an agent, not a human, is the one about to edit a widely-used shared source.
- **Template gallery** — workspace-local document and block templates: preview, describe, and insert a copy (a one-time starting point, not a live synced reference — see synced-block provenance above for that distinction). Seed a small set matching common knowledge-work patterns (meeting note, project brief, research brief, decision record, task collection/dashboard). Deliberately deferred past 0.4.0 to keep that release focused on editor/collection-view coherence rather than adding a fifth new content concept at once; tracked in the backlog for whenever it's next prioritized.
- **Full-width block display** — an individual block (notably an embedded Kanban/Calendar collection view) can opt out of the page's normal content-width column and use the full page width, the way Notion treats full-width as a per-block/per-page setting.
...
- **Multi-select property type** (open-vocabulary, multi-value analog to Select) — enables free-form tagging use cases; concretely tracked in #102. Deliberately deferred rather than designed now, the same way the formula property type above is reserved but undesigned — kept on the roadmap rather than ruled out, since tagging is a common and valuable pattern once the Collection model matures. Note: #93 scoped multi-select out as a non-goal for its own narrower field-manager work, not as a permanent product decision.
- **Edgeless/whiteboard canvas mode** (AFFiNE-style freeform spatial canvas, in the spirit of tools like Lucidchart). Explicitly not a block type or view — it's a second document model (freeform spatial x/y positioning) that doesn't fit the Core Architectural Principle's "Document is an ordered sequence of block-records," unlike every other block-type addition in this spec, so it can't be added as a simple extension of the existing block/view model. Genuinely of interest — the founder uses tools like this regularly at work — but deliberately deferred past 0.4.0 rather than scoped now (2026-09-01): the Diagram/Mermaid block (P1, Editor completeness) covers a large share of the same need (technical diagrams embedded in documents) at a fraction of the architectural cost, and is worth trying first. Revisit this item once that's shipped and it's clear how much of the actual need it does or doesn't cover.
