...
- **Columns/layout block** — a multi-column container for arranging block content side-by-side on a page, validated as a near-universal editor feature: Notion's column blocks, Confluence's page-layout/panel system, and WordPress Gutenberg's Columns block all converge on it independently. Requires block-model support for nested child-block containers — the first block type to need it; toggle/collapsible's own collapse today only hides/shows via a UI affordance, not real child containment, and would need a follow-up to actually nest its children the same way _(corrected 2026-09-06: this item previously and incorrectly claimed columns would reuse an existing toggle nesting pattern)_. Must degrade gracefully on narrow viewports (columns stack) per the accessibility/responsive acceptance bar (see Requirements intro).
