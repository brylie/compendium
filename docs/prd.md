...
- **Columns/layout block** — a multi-column container for arranging block content side-by-side on a page, validated as a near-universal editor feature: Notion's column blocks, Confluence's page-layout/panel system, and WordPress Gutenberg's Columns block all converge on it independently. Requires block-model support for nested child-block containers, following the same pattern toggle/collapsible already establishes. Must degrade gracefully on narrow viewports (columns stack) per the accessibility/responsive acceptance bar (see Requirements intro).
