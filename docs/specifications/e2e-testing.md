...
| `listDocuments`/`listCollections`/`searchWorkspace` scoped to one Space → never returns another Space's content, even when both contain matching text; composes correctly with per-ID token scoping (#133) — currently covered as a server-project unit test (`src/lib/server/space-isolation.test.ts`), not yet a Tier A protocol-level test, since there's no MCP-exposed Space selector yet (that's #6's product-surface job) | `workspace-sharding.md` §9: "UI and MCP operations enforce the same catalog, Space, and shard scope"; PRD's multi-space non-goal-adjacent requirement (#6)                                                                                                                                                                                                                                                                                                          |
