...
- **Callout.** Subtle border (`border-border`, unstyled), rounded (10px). A callout with no style set keeps the original single appearance (`bg-surface`, icon in `accent`, default `fg` body text) — this remains the default for a fresh callout and for any pre-#42 one. A callout _with_ a chosen style (§6) instead tints its background, icon, and body text together as one coherent unit (all three the same computed color, not icon-only) — the four presets via fixed tokens, a custom choice via per-record CSS custom properties.
