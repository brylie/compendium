...
**`viewConfig`'s merge granularity:** like `checked`/`collapsed`/`referencedRecordId`, `viewConfig` is written as one whole-value `Y.Map.set` (`setRecordViewConfig` in `src/lib/data/records.ts`), not decomposed into nested Yjs types per member. Two concurrent edits to _different_ `viewConfig` members (one actor changes `filters`, another changes `sort`, at the same time) resolve via last-write-wins on the whole object — one edit can be silently dropped, unlike a Collection row's `properties`, which merge per-property. Accepted for the same reason `checked`/`collapsed` are: a `collection_view` block's config isn't typically co-edited concurrently the way row properties are. Revisit if that assumption proves wrong in practice (tracked in #71).
