...
interface WorkspaceRecord {
	...
	content?: RichText; // set when parent is a Document — the block's text; absent for a content-less container block (columns/column, §3.1), which holds no text of its own; a toggle (issue #227) is a container that *does* keep this — its own summary line
	...
	// issue #148; toggle, issue #227 — see §3.1) — its own child records'
	// ids, in order, one level below this record rather than in its owning
	// Document's own recordIds. Its mere presence, not a hardcoded blockType
	// check, is what makes a record a valid `parentId` target — see
	// parentKindOf.
...
- `synced_block` references the source record through `referencedRecordId`; editing or holding the rendered block operates on that source record rather than an independent copy. Content convergence itself needs no bespoke propagation logic — a `synced_block` carries its own (unused) `content` field but the UI resolves it through the source's `Y.Text` instead (`syncedBlockTargetId` in `+page.svelte`), so both locations are the same shared CRDT text.
  ...
  - **Shard-aware resolution (issue #242):** a `synced_block`'s target is never assumed to live in the viewing Document's own shard. `getRecordYText` only ever sees the shard it's handed, so client-side rendering first tries the local `ydoc`; on a miss, it resolves the target's real shard via `GET /api/records/[id]/shard` and connects to it (`resolveRecordDoc`, `src/lib/client/yjs-client.ts`), mirroring the pre-existing cross-Collection pattern `resolveCollectionDoc` uses for relation properties (issue #204). That resolved connection's own Awareness is also what a cross-shard target's hold visibility comes from — `heldByOthers` only ever reflects the _viewing_ Document's own shard, so a synced_block whose target is held elsewhere merges in a hold observed on the target's own resolved Awareness instead (`createSyncedBlockResolver`, `src/lib/client/synced-block-resolution.svelte.ts`). "Used in N places" is answered the same way one layer down: `listIncomingLinks`/`listSyncedBlockInstances` (`links.ts`) stay a same-`Y.Doc` (same-shard) live index by construction — a `WeakMap<Y.Doc, …>` structurally can't see another shard — so a durable, cross-shard reverse index (`synced_block_instance`, `db/schema.ts`; kept live by `synced-block-index-observer.ts` the same way `record_index` is) backs a batched `POST /api/sync-groups` lookup (`services/synced-blocks.ts#resolveSyncGroups`) that the client fetches once per Document view and merges with the still-live local index, rather than replacing it.
...
### 3.1 Container blocks: real nesting (`columns`/`column`, issue #148; `toggle`, issue #227)
...
**`toggle` (issue #227) extends the same mechanism one type further, with one deliberate difference from `columns`/`column`: it keeps its own `content` in addition to its `childRecordIds`** — `BLOCK_CAPABILITIES.toggle` is the one entry with both `isContainer: true` and `holdsFreeformText: true`, which is what `record-ops.ts`'s `applyDocumentKindFields` and `services/records.ts`'s markdown-write guard key off (`isContainer && !holdsFreeformText`, not `isContainer` alone) to know a container has no content of its own. A toggle's own curated child-type subset (`BLOCK_CAPABILITIES.toggle.childBlockTypes`, `types.ts`'s `toggleChildBlockTypes`) is, for now, the same list `column` uses — no container/reference types, and no toggle-in-toggle nesting, keeping actual nesting depth the same one level `columns`/`column` already established rather than opening up unbounded recursion. A fresh toggle starts with zero children (unlike a fresh `columns` block's pre-seeded columns) — an empty child list is the normal, non-breaking state for both a newly created toggle and every toggle that existed before this issue shipped. Converting a toggle's blockType away from `toggle` (`setBlockType`) promotes its children to siblings immediately after it rather than deleting them, since nothing about "no longer a toggle" implies "children should be destroyed." The UI hides a toggle's children while `collapsed` is true (`ToggleChildren.svelte`, `+page.svelte`'s `insideContent`), but `get_document`'s markdown always renders the full children regardless of `collapsed` — matching native `<details>` semantics, where the content exists in the markup whether or not `open` is set — so an agent reading a collapsed toggle is never silently shown less than a human editing it would eventually see (`markdown-transcoding.md`).
