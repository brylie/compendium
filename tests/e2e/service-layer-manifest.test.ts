...
describe('Service Layer Manifest Wiring', () => {
	...
	it('10. Service layer manifest wiring check: validates all MCP tools and UI surface side effects', async () => {
		...
		const testDoc = serviceModules.documents.createDocument(resolveRequestContext(human), {
			title: 'Manifest Wiring Doc',
			createInitialBlock: true
		});
		const testBlock = serviceModules.records.createRecord(resolveRequestContext(human), {
			parentId: testDoc.id,
			blockType: 'paragraph'
		});
		const testCol = serviceModules.collections.createCollection(resolveRequestContext(human), {
		...
		for (const method of methods) {
			...
			switch (method) {
				case 'documents.createDocument': {
					const d = serviceModules.documents.createDocument(resolveRequestContext(human), {
						title: 'Wiring Sub'
					});
				...
				case 'documents.updateDocumentTitle': {
					serviceModules.documents.updateDocumentTitle(
						resolveRequestContext(human),
						testDoc.id,
						'Renamed Doc'
					);
				...
				case 'documents.listDocuments': {
					const list = serviceModules.documents.listDocuments(resolveRequestContext(human));
					expect(list.some((d) => d.id === testDoc.id)).toBe(true);
					break;
				}
				case 'documents.listBacklinks': {
					const linked = serviceModules.documents.createDocument(resolveRequestContext(human), {
					...
					const backlinks = serviceModules.documents.listBacklinks(
						resolveRequestContext(human),
						testDoc.id
					);
				...
				case 'documents.deleteDocument': {
					const toDelete = serviceModules.documents.createDocument(resolveRequestContext(human), {
						title: 'To Delete'
					});
					serviceModules.documents.deleteDocument(resolveRequestContext(human), toDelete.id);
				...
				case 'records.createRecord': {
					const r = serviceModules.records.createRecord(resolveRequestContext(human), {
						parentId: testDoc.id,
						blockType: 'paragraph'
					});
				...
				case 'records.writeRecord': {
					serviceModules.records.writeRecord(resolveRequestContext(human), testBlock.id, {
				...
				case 'records.deleteRecord': {
					...
					serviceModules.records.deleteRecord(resolveRequestContext(human), r.id);
					const log = queryAuditLog().filter((e) => e.targetRecordId === r.id);
				...
				case 'collections.listCollections': {
					const list = serviceModules.collections.listCollections(resolveRequestContext(human));
					expect(list.some((c) => c.id === testCol.id)).toBe(true);
					break;
				}
				case 'collections.updateCollectionTitle': {
					serviceModules.collections.updateCollectionTitle(
						resolveRequestContext(human),
						testCol.id,
						'Renamed Col'
					);
				...
				case 'collections.deleteCollection': {
					...
					serviceModules.collections.deleteCollection(resolveRequestContext(human), col.id);
					const log = queryAuditLog().filter((e) => e.targetRecordId === col.id);
					expect(log.some((e) => e.action === 'delete_collection')).toBe(true);
					break;
				}
				case 'spaces.createSpace': {
					const space = serviceModules.spaces.createSpace(
						resolveRequestContext(human),
						'Manifest Wiring Space'
					);
				...
				case 'tokens.revokeToken': {
					...
					serviceModules.tokens.revokeToken(resolveRequestContext(human), record.tokenHash);
					const log = queryAuditLog().filter((e) => e.targetRecordId === record.tokenHash);
				...
				case 'export.exportWorkspace': {
					const res = serviceModules.export.exportWorkspace(resolveRequestContext(human));
					expect(res.manifest).toBeDefined();
					expect(res.zipBuffer).toBeDefined();
					break;
				}
				case 'export.exportDocument': {
					const docs = serviceModules.documents.listDocuments(resolveRequestContext(human));
					if (docs.length > 0) {
						const res = serviceModules.export.exportDocument(
							resolveRequestContext(human),
							docs[0].id
						);
				...
				case 'export.exportCollection': {
					const cols = serviceModules.collections.listCollections(resolveRequestContext(human));
					if (cols.length > 0) {
						const res = serviceModules.export.exportCollection(
							resolveRequestContext(human),
							cols[0].id
						);
				...
				case 'export.updateMirrorConfig': {
					const updated = serviceModules.export.updateMirrorConfig(resolveRequestContext(human), {
						enabled: false
					});
