...
describe('export service', () => {
	...
	it('exports a full Workspace bundle with manifest.json and a valid ZIP buffer', () => {
		...
		expect(manifestStr).toContain('Nested Child Document');
	});
	...
	it('handles relative mirror outputDir and cleans empty directories', () => {
		...
		const staleSubDir = path.join(absDir, 'stale-folder', 'nested');
		fs.mkdirSync(staleSubDir, { recursive: true });

		// Syncing again should delete the empty stale subdirectories
		syncMarkdownMirror();
		expect(fs.existsSync(staleSubDir)).toBe(false);
