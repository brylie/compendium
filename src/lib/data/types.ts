...
// status, non-HTML response, or a guard in $lib/server/link-preview.ts). The
// block's own `url` field is always present once configured regardless of
// status, so a plain accessible link is never lost in either the 'pending'
// or 'error' state — see markdown-transcoding.md and BookmarkBlock.svelte.
