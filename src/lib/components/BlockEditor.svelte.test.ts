...
describe('BlockEditor', () => {
	...
	// "Pasting a URL offers a rich preview card" (issue #155) — dispatched as a
	// raw DOM event (not @testing-library/svelte's fireEvent.paste) so
	// clipboardData can be attached directly; jsdom's ClipboardEvent
	// constructor doesn't accept it as a constructor option.
	...
	it('calls onPasteUrl and prevents the default paste when pasting a bare URL into an empty block', () => {
		const ytext = doc.getText('a');
		const onPasteUrl = vi.fn(() => true);
		const { container } = render(BlockEditor, { ytext, ...handlers(), onPasteUrl });
		const el = container.querySelector('[contenteditable]') as HTMLElement;

		const prevented = pasteText(el, 'https://example.com/page');

		expect(onPasteUrl).toHaveBeenCalledWith('https://example.com/page');
		expect(prevented).toBe(true);
	});

	it('does not call onPasteUrl when the block already has text', () => {
		const ytext = doc.getText('a');
		...
		pasteText(el, 'https://example.com/page');

		expect(onPasteUrl).not.toHaveBeenCalled();
	});

	it('does not call onPasteUrl for pasted text that is not a single bare URL', () => {
		const ytext = doc.getText('a');
		const onPasteUrl = vi.fn(() => true);
		const { container } = render(BlockEditor, { ytext, ...handlers(), onPasteUrl });
		const el = container.querySelector('[contenteditable]') as HTMLElement;

		pasteText(el, 'check out https://example.com/page it is great');

		expect(onPasteUrl).not.toHaveBeenCalled();
	});

	it('leaves the default paste behavior alone when onPasteUrl is not provided', () => {
		const ytext = doc.getText('a');
		const { container } = render(BlockEditor, { ytext, ...handlers() });
		const el = container.querySelector('[contenteditable]') as HTMLElement;
