...
describe('routes/calendar/[id]/+page.server', () => {
	it('returns the collection title for an existing collection', () => {
		const doc = getYDoc();
		const collection = createCollection(doc, { title: 'My Calendar', schema: [] });

		const result = load({ params: { id: collection.id } } as Parameters<typeof load>[0]);

		expect(result).toEqual({ collectionId: collection.id, title: 'My Calendar' });
	});
