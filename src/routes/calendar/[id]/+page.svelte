<script lang="ts">
	...
	let datePropertyKey: string | undefined = $state();
	let viewYear = $state(today.getFullYear());
	let viewMonth = $state(today.getMonth()); // 0-11

	const dateProperties = $derived(schema.filter((p) => p.type === 'date'));
	const dateProperty = $derived(schema.find((p) => p.key === datePropertyKey));
	...
	const entryFields = $derived(
		visibleProperties(schema, config).filter(
			(p) => p.key !== datePropertyKey && p.key !== titlePropertyKey
		)
	);
	...
	function refresh(): void {
		...
		rows = view.records;
		if (!datePropertyKey || !schema.some((p) => p.key === datePropertyKey && p.type === 'date')) {
			datePropertyKey = schema.find((p) => p.type === 'date')?.key;
		}
	}
...
<div class="mx-auto max-w-7xl px-6 py-10">
	...
	<ViewSwitcher collectionId={data.collectionId} active="calendar" />

	{#if dateProperties.length === 0}
		<div class="rounded-lg border border-dashed border-border bg-surface/50 p-8 text-center">
			<p class="text-sm text-muted">
				Calendar view places records by a <strong>date</strong> property, and this collection doesn't
				have one yet.
			</p>
			<button
				type="button"
	...
	{:else}
		<div class="mb-4 flex flex-wrap items-center gap-3">
			<label class="text-xs text-muted" for="calendar-date-property">Dates from</label>
			<select
				id="calendar-date-property"
				bind:value={datePropertyKey}
