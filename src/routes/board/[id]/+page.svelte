<script lang="ts">
	...
	let config: ViewConfig = $state({ sort: { mode: 'manual' } });
	let groupPropertyKey: string | undefined = $state();
	...
	const groupProperty = $derived(schema.find((p) => p.key === groupPropertyKey));
	...
	const cardFields = $derived(
		visibleProperties(schema, config).filter(
			(p) => p.key !== groupPropertyKey && p.key !== titlePropertyKey
		)
	);
	...
	function refresh(): void {
		...
		if (
			!groupPropertyKey ||
			!schema.some((p) => p.key === groupPropertyKey && p.type === 'select')
		) {
			groupPropertyKey = schema.find((p) => p.type === 'select')?.key;
		}
	}
	...
	function cardTitle(row: WorkspaceRecord): string {
		const titleProperty = schema.find((p) => p.type === 'text');
...
<div class="mx-auto max-w-7xl px-6 py-10">
	...
	<ViewSwitcher collectionId={data.collectionId} active="board" />
	...
	{:else}
		<div class="mb-4 flex items-center gap-2">
			<label class="text-xs text-muted" for="board-group-property">Group by</label>
			<select
				id="board-group-property"
				bind:value={groupPropertyKey}
