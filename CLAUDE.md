...
2. **Check PRD alignment.** Confirm the work fits the PRD's goals/non-goals and current phase (`docs/prd.md`); if it doesn't cleanly fit, that's a signal to either scope the work down or raise the mismatch rather than silently expanding product surface. If implementing the issue reveals the PRD itself needs a small correction or addition to stay accurate, make that edit too — the PRD is meant to track reality, not go stale. For a bigger product-direction question (does this cross a stated Non-Goal, should a P1/P2 item be pulled forward, is it time to open the next phase) rather than a quick per-issue check, use the `product-owner` skill instead of deciding it inline.
