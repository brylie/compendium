...
2. **Check PRD alignment.** Confirm the work fits the PRD's goals/non-goals and current phase (`docs/prd.md`); if it doesn't cleanly fit, that's a signal to either scope the work down or raise the mismatch rather than silently expanding product surface. If implementing the issue reveals the PRD itself needs a correction or addition — however small — to stay accurate, use the `product-owner` skill to propose it: show the exact diff and wait for approval before writing it, the same as any bigger product-direction question (does this cross a stated Non-Goal, should a P1/P2 item be pulled forward, is it time to open the next phase). Don't edit `docs/prd.md` directly from inside an unrelated implementation task, even for a "small" fix.
...
SvelteKit UI  ◄──/ws (y-websocket)──►  Y.Doc (in-memory, per workspace/shard context)  ◄──/mcp (HTTP)──►  MCP client
                                   ...
                                   access_tokens, record_index (FTS5 —
                                   backs search_workspace; query_collection's
                                   filter reads the Y.Doc directly instead —
                                   see persistence.md §2)
```
...
- **Persistence** (`persistence.md`): Drizzle/SQLite owns `snapshots` (periodic `Y.encodeStateAsUpdate` binary dumps, keyed by `(workspace_id, shard_id)` and loaded lazily the first time a given workspace/shard context is resolved, not all at once on process start), `audit_log` (append-only), and `access_tokens`. Persistence.md §2's `record_index` — a one-directional, disposable SQLite FTS5 projection of the `Y.Doc`, kept live per-shard by `src/lib/server/record-index-observer.ts` and rebuilt wholesale on every shard load (`src/lib/server/record-index.ts`) — is built and backs `search_workspace`'s full-text matching (it's a raw-SQL virtual table created in `src/lib/server/db/index.ts`, not `db/schema.ts`, since drizzle-kit has no representation for FTS5). `query_collection`'s `filter` deliberately does **not** read from it: it applies the same `applyFilters`/`ViewFilter[]` semantics the UI's Table/Board/Calendar views use directly against the already-resolved single-shard `Y.Doc` records — real UI/MCP parity without a SQLite dependency on that path (see `collection-views.md` §8, `persistence.md` §2). The Table view's live grid was always meant to read directly off Yjs observers regardless, not this table.
